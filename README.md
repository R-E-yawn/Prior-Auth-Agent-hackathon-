...
| **Kiro** | AWS’s agentic IDE — plan before code | Primary **development workflow**: spec-first design (`lib/types.ts`, SSE contract, orchestrator), multi-file edits, and iteration from **plan → execution**. See **`KIRO_WRITEUP.md`** (and **`KIRO_WRITEUP.docx`**) in the repo root for judges. |
